...
void Reset_oneAxisSolarArrayPoint(OneAxisSolarArrayPointConfig *configData, uint64_t callTime, int64_t moduleID)
{
    ...
    else if (v3Norm(configData->h1Hat_B) > SOLAR_ARRAY_EPS) {
    ...
    if (InertialHeadingMsg_C_isLinked(&configData->inertialHeadingInMsg)) {
        ...
        if (v3Norm(configData->hHat_N) > SOLAR_ARRAY_EPS) {
...
void Update_oneAxisSolarArrayPoint(OneAxisSolarArrayPointConfig *configData, uint64_t callTime, int64_t moduleID)
{
    ...
    double a2Hat_B[3];
    if (v3Norm(configData->a2Hat_B) > SOLAR_ARRAY_EPS) {
    ...
    if (v3Norm(configData->h2Hat_B) > SOLAR_ARRAY_EPS) {
        ...
        if (dotP_2 > dotP_1 && fabs(dotP_2 - dotP_1) > SOLAR_ARRAY_EPS) {
...
void computeFirstRotation(double hRefHat_B[3], double hReqHat_B[3], double R1B[3][3])
{
    ...
    if (fabs(phi-MPI) < SOLAR_ARRAY_EPS) {
        phi = MPI;
        v3Perpendicular(hRefHat_B, e_phi);
    }
    else if (fabs(phi) < SOLAR_ARRAY_EPS) {
        phi = 0;
    }
    // normalize e_phi
...
void computeSecondRotation(double hRefHat_B[3], double rHat_SB_R1[3], double a1Hat_B[3], double a2Hat_B[3], double R2R1[3][3])
{
    ...
    if (fabs(A) < SOLAR_ARRAY_EPS) {
        if (fabs(B) < SOLAR_ARRAY_EPS) {
        ...
        if (Delta < 0) {
            ...
            t = t1;            
            if (fabs(v3Dot(hRefHat_B, a2Hat_B)-1) > SOLAR_ARRAY_EPS) {
                y1 = (E*t1*t1 + F*t1 + G) / (1 + t1*t1);
                y2 = (E*t2*t2 + F*t2 + G) / (1 + t2*t2);
                if (y2 - y1 > SOLAR_ARRAY_EPS) {
                    t = t2;
                }
            }
            psi = 2*atan(t);
...
void computeThirdRotation(int alignmentPriority, double hRefHat_B[3], double rHat_SB_R2[3], double a1Hat_B[3], double R3R2[3][3])
{
    ...
    if (alignmentPriority == prioritizeAxisAlignment) {
        ...
        if (fabs(theta) < SOLAR_ARRAY_EPS) {
            ...
            if (fabs(fabs(theta)-MPI/2) > SOLAR_ARRAY_EPS) {
                for (int i = 0; i < 3; i++) {
                ...
                if (v3Norm(aPHat_B) < SOLAR_ARRAY_EPS) {
